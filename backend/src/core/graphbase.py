...
class GraphDatabase:
    ...
    def start(self):
        uri = self.config.neo4j.get("uri", "neo4j://localhost:19013") # os.environ.get("NEO4J_URI", "neo4j+s://3fbd4d0e.databases.neo4j.io")
        username = self.config.neo4j.get("username", "neo4j") #os.environ.get("NEO4J_USERNAME", "neo4j")
        password = self.config.neo4j.get("password", "0123456789")    #os.environ.get("NEO4J_PASSWORD", "UyNKpUBso-LGEhJjT3LmVQk-bJKOEoI6j_dQ_OTvSWc")
        ...
        try:
            ...
            logger.error(f"Failed to connect to Neo4j: {e}, {uri}, neo4j, {username}, {password}")
